...
namespace LogCallsAnalyzer.Parser
{
    ...
    public sealed class AnalyzerConfig
    {
        ...
        private static readonly Regex _sectionMatcher = new(@"^\s*\[(([^#;]|\\#|\\;)+)\]\s*([#;].*)?$", RegexOptions.Compiled);
        // Matches EditorConfig property such as "indent_style = space", see http://editorconfig.org for details
        private static readonly Regex _propertyMatcher = new(@"^\s*([\w\.\-_]+)\s*[=:]\s*(.*?)\s*([#;].*)?$", RegexOptions.Compiled);
        ...
        /// not be removed.
        /// </remarks>
        internal static ImmutableHashSet<string> ReservedKeys { get; } = ImmutableHashSet.CreateRange(Section.PropertiesKeyComparer,
            new[] { "root", "indent_style", "indent_size", "tab_width", "end_of_line", "charset", "trim_trailing_whitespace", "insert_final_newline", });
        ...
        /// </summary>
        internal static Comparer<AnalyzerConfig> DirectoryLengthComparer { get; } = Comparer<AnalyzerConfig>.Create((e1, e2) => e1.NormalizedDirectory.Length.CompareTo(e2.NormalizedDirectory.Length));
        ...
        /// </summary>
        public bool IsRoot => GlobalSection != null && GlobalSection.Properties.TryGetValue("root", out string val) && val == "true";
        ...
        private AnalyzerConfig(Section? globalSection, ImmutableArray<Section> namedSections, string pathToFile)
        {
            ...
            NormalizedDirectory = NormalizeWithForwardSlash(directory);
        }

        private static readonly char _directorySeparatorChar = RuntimeInformation.IsOSPlatform(OSPlatform.Linux) ? '/' : '\\';
        /// <summary> If the current environment uses the '\' directory separator, replaces all uses of '\' in the given string with '/'. Otherwise, returns the string.
        /// </summary>
        /// <remarks>
        /// This method is equivalent to Microsoft.CodeAnalysis.BuildTasks.GenerateMSBuildEditorConfig.NormalizeWithForwardSlash. Both methods should be kept in sync.
        /// </remarks>
        private static string NormalizeWithForwardSlash(string p)
            => _directorySeparatorChar == '/' ? p : p.Replace(_directorySeparatorChar, '/');
        ...
        /// </summary>
        public static AnalyzerConfig Parse(string text, string pathToFile) => Parse(SourceText.From(text), pathToFile);
